              ...
              <div class="panel-body">
                ...
                <div class="col-sm-12 form-type">
                  <div class="form-group">
                    ...
                    <textarea class="form-control mce_basic" name="description">{{$product->description}}</textarea>
                  </div>
                </div>
                ...
                <div class="col-sm-12 form-type">
                  <div class="form-group">
                    ...
                    <textarea class="form-control mce_full" name="content">{{$product->content}}</textarea>
                  </div>
                </div>
              ...
              <div class="panel-body">
                ...
                <div class="col-sm-12 form-type">
                  <div class="form-group">
                    ...
                    <input name="tags" id="tags-input" data-role="tagsinput" data-tag-class="label label-primary" class="form-control" value="{{$product->tags}}">
                  </div>
                </div>
                ...
                <div class="col-sm-12 form-type">
                  <div class="form-group">
                    <label class="control-label">Ảnh mô tả chính</label>
                      ...
                      <img src="{{$product->featured_image}}" id="featured_image_src" class="thumbnail" width="100%">
                ...
                <div class="col-sm-12 form-type">
                  <div class="form-group">
                    <label class="control-label">Ảnh mô tả phụ</label>
                      <input type="hidden" name="thump_image" id="featured_icon" value="{{$product->thump_image}}">
                      <img src="{{$product->thump_image}}" id="featured_icon_src" class="thumbnail" width="100%">
                      ...
                      @foreach($categories_list as $cp)
                        @if($cp->parent == 0)
                          @foreach($categories_list as $cp2)
                            @if($cp2->parent == $cp->id)
                            <option value="{{$cp2->id}}" @if($product->parent == $cp2->id){{'hidden selected'}}@endif>{{$cp2->title}}</option>
                            @foreach($categories_list as $cp3)
                              @if($cp3->parent == $cp2->id)
                              <option value="{{$cp2->id}}" @if($product->parent == $cp3->id){{'hidden selected'}}@endif>-- &nbsp;&nbsp;{{$cp3->title}}</option>
                            @endif
                          @endforeach
                            @endif
                          @endforeach
                ...
                <div class="col-sm-12 form-type">
                  <div class="form-group">
                    ...
                    <input type="number" class="form-control" name="quantity" min="0" step="1" value="{{$product->quantity}}" required="">
                ...
                <div class="col-sm-12 form-type">
                  <div class="form-group">
                    ...
                    <input type="number" class="form-control" name="origin_price" min="0" step="any" value="{{$product->origin_price}}" required="">
                ...
                <div class="col-sm-12 form-type">
                  <div class="form-group">
                    ...
                    <input type="number" class="form-control" name="sale" max="100" min="0" step="any" value="{{$product->sale}}" required="">
                    @if($errors->has('sale'))
                ...
                <div class="col-sm-12 form-type">
                  <div class="form-group">
                    ...
                    <select id="status" name="status" class="form-control">
                      <option value="public" @if($product->status == 'public'){{'selected'}}@endif>Hiện</option>
                      <option value="preview" @if($product->status == 'preview'){{'selected'}}@endif>Ẩn</option>
                    </select>
                  </div>
                </div>
                ...
                <div class="col-sm-12 form-type center">
                  <div class="form-group">
                    <br>
                    <a href="{{$product->slug}}" class="btn btn-info" target="_blank">Link sản phẩm</a>&emsp;
                    <button type="submit" class="btn btn-primary">OK</button>&emsp;
                    <a href="cms/san-pham/edit/{{$product->id}}" class="btn btn-default">Reset</a>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="content-with-menu-container">
              ...
              <div class="mg-main">
                <div class="row mg-files" data-sort-destination data-sort-id="media-gallery">
                  ...
                  <input type="hidden" class="total" value="{{count($product->slider)}}">
                  @if(count($product->slider) > 0)
                    @foreach($product->slider as $key => $slider)
                    <div class="isotope-item col-sm-6 col-md-4 col-lg-3" data-id="file_{{$key+1}}">
                      <div class="thumbnail">
                        <div class="thumb-preview">
                          <a class="thumb-image" id="slide_image_file_{{$key+1}}_href" href="{{$slider->image}}">
                            <img class="img-responsive" alt="Project" id="slide_image_file_{{$key+1}}_src" src="{{$slider->thumb}}">
                          </a>
                          <div class="mg-thumb-options">
                            <div class="mg-zoom"><i class="fa fa-search"></i></div>
                            <div class="mg-toolbar">
                              <div class="mg-option checkbox-custom checkbox-inline">
                                <input type="hidden" id="slide_image_file_{{$key+1}}" name="slider[]" value="{{$slider->image}}">
                                <input type="checkbox" id="file_{{$key+1}}" class="slide">
                                <label for="file_{{$key+1}}">CHỌN</label>
                              </div>
                              <div class="mg-group pull-right">
                                <a style="cursor: pointer;" onclick="uploadSlider('file_{{$key+1}}')" class="SliderUpload">THAY ĐỔI</a>
                                <button class="dropdown-toggle mg-toggle" type="button" data-toggle="dropdown">
                                  <i class="fa fa-caret-up"></i>
                                </button>
                                <ul class="dropdown-menu mg-menu" role="menu">
                                  <li><a style="cursor: pointer;" onclick="uploadSlider('file_{{$key+1}}')" class="SliderUpload"><i class="fa fa-edit"></i> Thay đổi</a></li>
                                  <li><a style="cursor: pointer;" onclick="delSlide('file_{{$key+1}}')" class="SliderUpload"><i class="fa fa-trash-o"></i> Xóa</a></li>
                                </ul>
                              </div>
                            </div>
                          </div>
