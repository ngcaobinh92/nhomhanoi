...
<html lang="vi">
  ...
  <body>
          ...
          <div class="row">
            <div class="col-xs-8 col-sm-8 hidden-xs">
              <div class="hotline_top">
                ...
                <b style="color:#fff;">Tư vấn 24/7:</b> <a href="tel:{{$configs->hotline}}">{{$configs->hotline}} </a>
              </div>
              <p class="diachi_header"><span>Địa chỉ:</span> {{$configs->address}}</p>
            </div>
            <div class="col-xs-12 col-sm-4 text-right">
              ...
              <div class="dropdown boxtaikhoan">
                ...
                @if(Auth::user())
                <i class="fa fa-user" aria-hidden="true"></i> Xin chào {{Auth::user()->name}} <i class="far fa-angle-down"></i></a>
              ...
              <div class="mini-cart dropdown box-cart cart hidden-xs">
                <a href="gio-hang" class="dropdown-toggle basket" data-toggle="dropdown" data-hover="dropdown">
                  ...
                  <span id="cart-total">{{count(@$cart)}}</span>
                </a>
                  ...
                  <table class="table table-condensed cart_info">
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td class="cart_price">
                          <p data-product-price="{{$item->id}}">{{number_format($item->price)}} đ</p>
                        </td>
                        <td class="cart_quantity">
                          <div class="cart_quantity_button">
                            <a class="product-jian" data-href='{{url("gio-hang?product_id=$item->id&decrease=1")}}'> - </a>
                            <span class="product-num" data-product='{{$item->id}}'/>{{$item->qty}}</span>
                            <a class="product-add" data-href='{{url("gio-hang?product_id=$item->id&increment=1")}}'> + </a>
                          </div>
                        </td>
                        <td class="cart_total">
                          <p class="cart_total_price" data-product-total='{{$item->id}}'>{{number_format($item->subtotal)}} đ</p>
                        </td>
                        <td>
                          <a class="cart_change" data-href='{{url("gio-hang?product_id=$item->id&remove=1")}}' data-product-id="{{$item->id}}"><i class="fa fa-times"></i></a>
                        </td>
                      </tr>
                      @endforeach
                    </tbody>
      ...
      <nav class="navbar menumain visible-xs mobile-menu">
        ...
        <div class="navbar-collapse collapse navbar-left">
          <ul class="nav navbar-nav list-collections list-cate-banner">
            ...
            <li class="menu_lv1 item-sub-cat"><a href="{{$category->slug}}">{{$category->title}}</a></li>
            @endforeach
        ...
        <form action="tim-kiem" method="get" class="navbar-form navbar-search navbar-right hidden-md hidden-lg hidden-sm">
          <input name="ten" placeholder="Nhập thông tin cần tìm kiếm" class="search-query" maxlength="128" type="text" value="{{@$params['ten']}}">
          ...
          <nav class="navbar menumain">
            ...
            <div class="navbar-collapse collapse navbar-left">
              <ul class="nav navbar-nav">
                ...
                @foreach ($categories as $category)
                <li @if(count($category->child) > 0) {{'class=dropdown'}}@endif>
                  <a href="{{$category->slug}}">{{$category->title}} @if(count($category->child) > 0) <i class="fa fa-caret-down pull-right" aria-hidden="true"></i>@endif</a>
                  <ul class="dropdown-menu sub1">
                        ...
                        <li class="col-xs-12 col-sm-6">
                          <a href="{{$child->slug}}">{{$child->title}}
                            ...
                            <ul class="sub2">
                              ...
                              <li>
                                <a href="{{$grandchild->slug}}">{{$grandchild->title}}
                              </li>
                              @endforeach
                            </ul>
                            @endif
                    ...
                    <div class="col-xs-12 col-sm-4 hidden-xs">
                      <div class="box-bestseller">
                        ...
                        @if(count($hot) > 0)
                          @foreach($hot as $hot_product)
                          <div class="bestseller_one">
                            <a href="{{$hot_product->slug}}" title="{{$hot_product->title}}" class="bestseller_one_img">
                              <img src="{{$hot_product->featured_image}}" alt="{{$hot_product->title}}">
                            </a>
                            <h3 class="bestseller_one_name">
                              <a href="san-pham" title="{{$hot_product->title}}">{{$hot_product->title}}</a>
                            </h3>
                            <div data-id="{{$hot_product->id}}"></div>
            ...
            <form action="tim-kiem" method="get" class="navbar-form navbar-search navbar-right hidden-xs">
              <input name="ten" placeholder="Nhập thông tin cần tìm kiếm tại đây" class="search-query" maxlength="128" type="text" value="{{@$params['ten']}}">
              <button type="submit" class="btn icon-search"><i class="fa fa-search" aria-hidden="true"></i></button>
            </form>
          </nav>
        </div>
          ...
          <div class="so-maps">
            <div class="module google-map">
              <div class="modcontent clearfix">
                <div class="contact">
                  <div style="width: 100%; height: 450px;">{!!$configs->google_map!!}</div>
                  <div class="contact-info">
                    ...
                    <div class="address"> 
                      ...
                      @if($configs->hotline != '') <strong>Điện thoại:</strong> {{$configs->hotline}}<br>@endif
                      @if($configs->email != '') <strong>Email:</strong>  {{$configs->email}}<br>@endif
                      @if($configs->address != '') <strong>Địa chỉ :</strong> {{$configs->address}}<br>@endif
                      @if($configs->showroom != '') <strong>Showroom:</strong> {!!$configs->address!!}<br>@endif
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div class="container">
          <div class="row">
            <div class="col-xs-12 col-sm-4 social-box">
              ...
              @if($configs->facebook != '') 
                <li class="facebook"><a class="_blank" href="https://facebook.com/{{$configs->facebook}}" target="_blank"><i class="fab fa-facebook-f"></i></a></li>
              @endif
              @if($configs->twitter != '') 
                <li class="twitter"><a class="_blank" href="https://twitter.com/{{$configs->twitter}}" target="_blank"><i class="fab fa-twitter"></i></a></li>
              @endif
              @if($configs->rss != '') 
                <li class="rss"><a class="_blank" href="rss" target="_blank"><i class="fas fa-rss"></i></a></li>
              @endif
              @if($configs->google_plus != '') 
                <li class="google_plus"><a class="_blank" href="#" target="_blank"><i class="fab fa-google-plus"></i></a></li>
              @endif
              @if($configs->pinterest != '') 
                <li class="pinterest"><a class="_blank" href="https://www.pinterest.com/{{$configs->pinterest}}" target="_blank"><i class="fab fa-pinterest"></i></a></li>
              @endif
              @if($configs->zalo != '') 
                <li class="zalo"><a class="_blank" href="https://zalo.me/{{$configs->zalo}}" target="_blank"><div class="fa-zalo">Zalo</div></a></li>
              @endif
              </ul>
            </div>
            <div class="col-xs-12 col-sm-8">
              <ul class="list-ft pull-right">
                ...
                @foreach ($categories as $category)
                <li class="col-xs-6"><a href="{{$category->slug}}">{{$category->title}}</a></li>
                @endforeach
              </ul> 
            </div>
          </div>
          ...
          <div class="modal-body" id="fast-login">
            <div class="container-fluid">
                <div class="regist_right col-md-6">
                  ...
                  <a href="https://oauth.zaloapp.com/v4/oa/permission?app_id=2095051378983041256&redirect_uri=https%3A%2F%2Fnhomhanoi.svtest.info%2Fauth%2Fzalo%2Fcallback">
    ...
    <a href="tel:{{$configs->hotline}}" class="hotline-fixed-mobile"><i class="fa fa-phone ringing"></i> {{$configs->hotline}}</a>
