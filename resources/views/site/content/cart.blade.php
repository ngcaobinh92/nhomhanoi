  ...
  <div class="row content-page cart">
    <div class="col-md-12">
        <div class="product col-md-5 col-lg-5">
          ...
          <a class="delete-all cart_delete" data-href='{{url("gio-hang?destroy=1")}}'>Xóa toàn bộ</a>
          ...
          <div class="product-box">
            ...
            <div class="product-sx">
              <a href="{{$item->options->slug}}">
                <img src="{{$item->options->image}}" class="product-img" />
                <span class="product-name">{{$item->name}}</span>
              </a>
              <span class="product-price"><span class="price-cart">{{number_format($item->price)}}</span>đ</span>
              <div class="product-amount">
                <div class="product_gw">
                  <a class="product-jian" data-href='{{url("gio-hang?product_id=$item->id&decrease=1")}}'> - </a>
                  <input type="text" value="{{$item->qty}}" class="product-num" data-product='{{$item->id}}' id="product-{{$item->id}}-num" />
                  <a class="product-add" data-href='{{url("gio-hang?product_id=$item->id&increment=1")}}'> + </a>
                </div>
              </div>
              <div class="product-del">
                <a class="cart_change" data-href='{{url("gio-hang?product_id=$item->id&remove=1")}}' data-product-id="{{$item->id}}"><img src="public/img/deleteico.png" /></a>
              </div>
            </div>
          </div>
          @endforeach
          ...
          <div class="product-js col-md-6 col-lg-6">
            ...
            <div class="cart__body">
              <div class="col">
                <div class="mb-2">
                  ...
                  <div class="col-12">
                    <input type="text" name="order_cus_name" id="order_cus_name" class="form-control" placeholder="Nhập họ và tên" value="@auth{{Auth::user()->name}}@endauth" required="">
                  </div>
                </div>
                ...
                <div class="mb-2">
                  ...
                  <div class="col-12">
                    <input type="text" name="order_cus_phone" id="order_cus_phone" class="form-control" placeholder="Nhập số điện thoại" value="@auth{{Auth::user()->phone}}@endauth" pattern="^(0[3-9][0-9]{8})$" required="">
                  </div>
                </div>
                ...
                <div class="mb-2">
                  ...
                  <div class="col-12">
                    <textarea name="order_cus_address" id="order_cus_address" class="form-control" placeholder="Nhập địa chỉ đầy đủ: số nhà, tên đường" required="">@auth{{Auth::user()->address}}@endauth</textarea>
                  </div>
                </div>
            ...
            <div class="cart__footer flex-column flex-md-row">
              <div class="font-14">
                ...
                <label for="order_confirm">Tôi đồng ý với <a href="{{url('dieu-khoan-dich-vu')}}" target="__blank" class="text-primary fw-bold" title="Chính sách và điều khoản">&nbsp;chính sách và điểu khoản</a>&nbsp;của {{$_SERVER['SERVER_NAME']}} </label>
              </div>
